...
def get_torus_immersion(major_radius, minor_radius, embedding_dim, deformation_amp, translation, rotation):
    def immersion(angle_pair):
        ...
        amplitude2 = (
                1
                + deformation_amp * gs.exp(-2 * (phi - gs.pi / 2) ** 2) * gs.exp(-2 * (phi - gs.pi / 2) ** 2)
                + deformation_amp * gs.exp(-2 * (phi - 3 * gs.pi / 2) ** 2) * gs.exp(-2 * (phi - 3 * gs.pi / 2) ** 2)
            )
        ...
        if deformation_amp != 0.0 and embedding_dim > 5:
            point = gs.concatenate([point, gs.zeros(embedding_dim - 3)])
            point[3] = deformation_amp * amplitude2
            point[4] = deformation_amp * gs.cos(theta)
            point[5] = deformation_amp * gs.sin(2 * phi)
...
def get_sphere_immersion(radius, deformation_amp, embedding_dim, translation, rotation):
    def immersion(angle_pair):
        theta, phi = angle_pair
        amplitude1 = (
                1 + 0.1 * deformation_amp * ( gs.exp(-5 * theta ** 2)
        ...
        amplitude3 = 1 + 0.3 * (deformation_amp * torch.cos(theta) + deformation_amp * torch.cos(2 * theta))
        amplitude4 = 1 + 0.4 * (deformation_amp * torch.cos(phi) + deformation_amp * torch.cos(2 * phi))
...
def load_sphere(n_points, radius, noise_var, embedding_dim, deformation_amp, translation, rotation, random_seed=42):
    ...
    phis = gs.linspace(0 + eps, 2 * np.pi - eps, sqrt_ntimes)
    angle_grid = torch.cartesian_prod(thetas, phis)
