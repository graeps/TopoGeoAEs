...
def generate_toroidal_pointcloud(A, embed_dim, translation, num_points=80000, noise_std=0.00, device='cpu'):
    ...
    extra_dims = torch.zeros(num_points, embed_dim - 2 * d, device=device)
    ...
    if isinstance(translation, torch.Tensor) and translation.shape[0] == embed_dim:
        v_translation = translation
        print("pointcloud translated")
    else:
        v_translation = torch.zeros(embed_dim, device=device)
        print(
            f"pointcloud not translated. Translation invalid {translation}. Shape {translation.shape}, Instance {isinstance(translation, torch.Tensor)}")
